...
PShape createIcosahedronWithTex(int level, String tx) {
...
PVector[] getOrigVertArray(PShape ico) {
  PVector[] originalPos = new PVector[ico.getVertexCount()];
  for (int i = 0; i < ico.getVertexCount(); i++) {
    originalPos[i] = ico.getVertex(i); //to save the original positions
  }
  return originalPos; 
} 
...
PShape createIcosahedron(int level) { 
  ...
  for (int i = 0; i < ico.positions.size (); i++) {
    ...
    mesh.normal(n.x, n.y, n.z);
    mesh.fill(200); 
    //PVector oldVertex = new PVector(p.x, p.y, p.z);
    //oldVertex.add(n.x, n.y, n.z);
    //PVector newVertex = oldVertex;
    mesh.vertex(p.x, p.y, p.z);
    //mesh.vertex(newVertex.x, newVertex.y, newVertex.z);
  }
  mesh.endShape(); 
  return mesh;
} 
